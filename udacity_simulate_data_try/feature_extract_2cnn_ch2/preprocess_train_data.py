...
def make_hsv_grayscale_diff_data(path, num_channels=2):
    ...
    for i in range(num_channels, num_rows):
        ...
        for j in range(num_channels):
            # print("i: ", i)
            # print("j: ", j)
            path0 = df['center'].iloc[i - j - 1]
            path1 = df['center'].iloc[i - j]
    ...
    return X, np.array(df["steering"].iloc[num_channels:])
    
if __name__ == "__main__":
    config = DataConfig()
    # data_path = config.data_path
    row, col = config.img_height, config.img_width
    data_path ="/home/ubuntu/udacity_simulator_data/simulate_data/"

    print( "Pre-processing phase 2 data...")
    X_train, y_train = make_hsv_grayscale_diff_data("{}/driving_log.csv".format(data_path), 2)
    np.save("{}/X_train_round2_hsv_gray_diff_ch2_all_sim".format(data_path), X_train)
    np.save("{}/y_train_round2_hsv_gray_diff_ch2_all_sim".format(data_path), y_train)
