...
face_dataset = intrusion_data(csv_file='interpolated_attack_a_consecutive_0p25_1_5625.csv', root_dir='./data', cs=1,
                              transform=transforms.Compose(
                                  [transforms.Resize(256),
                                   transforms.RandomResizedCrop(224),
                                   transforms.ToTensor()]))

test_dataset = intrusion_data(csv_file='interpolated_attack_a_consecutive_0p25_1_2413.csv', root_dir='./data', cs=1,
                              transform=transforms.Compose(
                                  [transforms.Resize(256),
                                   transforms.RandomResizedCrop(224),
                                   transforms.ToTensor()]))
...
# &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&TESTING &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
    ...
    # print(filenm)
    torch.save(net, os.path.join(('saved_models_consecutive'), filenm))
