...
face_dataset = intrusion_data(csv_file='05-12_f10_a_consecutive_0p25_1_batch_11402.csv', root_dir='./data', cs=1,
                              transform=transforms.Compose(
                                  [transforms.Resize(256),
                                   transforms.RandomResizedCrop(224),
                                   transforms.ToTensor()]))

test_dataset = intrusion_data(csv_file='05-12_f10_a_consecutive_0p25_1_batch_4888.csv', root_dir='./data', cs=1,
                              transform=transforms.Compose(
                                  [transforms.Resize(256),
                                   transforms.RandomResizedCrop(224),
                                   transforms.ToTensor()]))
...
# &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&TESTING &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
    ...
    # print(filenm)
    torch.save(net, os.path.join(('models_consecutive-0p25-1'), filenm))
